        ...
        usersData.forEach(users => {
            powerCost.forEach(power => {
                if (users.timestamp == power.timestamp) {
                    totalUnitsUser += users.totalUnits * (power.hydro.cost + power.solar.cost + power.geothermal.cost);
                }
            })
        })
        ...
        officeData.forEach(office => {
            powerCost.forEach(power => {
                if (office.timestamp == power.timestamp) {
                    totalUnitsOffice += office.totalUnits * (power.hydro.cost + power.solar.cost + power.geothermal.cost);
                }
            })
        })
    } catch (err) {
        ...
        industryData.forEach(industry => {
            powerCost.forEach(power => {
                if (industry.timestamp == power.timestamp) {
                    totalUnitsIndustry += industry.totalUnits * (power.hydro.cost + power.solar.cost + power.geothermal.cost);
                }
            })
        })
